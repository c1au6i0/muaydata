...
# CREATE A METHOD FOR CLASS RESAMPLE
...
When we trained out models we select ```F``` as the metric to use to choose the optimal model. What is ```F```? F is the (harmonic) mean of 2 values: *Precision* and *Recall*.
The *precision* is the ratio of identified TruePositives / (TruePositives+FalsePositives), and thus indicates how likely is that a "Yes" predition is correct. Of course a model may be still be 100 % correct when it predicts "Yes" but be able to identify _*very few*_  "Yes". In that particular case the Recall (aka Sensitivity: TruePositives/AllPositive) of the model will be very low. The best possible model is able to correctly identify all the "Yes", and so has Precision = Recall = F = 1.

We can visually compare these measures of accuracy with Precision-Recall curves. This curves are generated by varying the cutoff probabilty for assigning the prediction values ("Yes" or "No") and by plotting Recall value vs Precision value for each cutoff value. Remember that perfection would generate a precision-recall curve that pass to the up-right corner of the grapha and thus have Precision = Recall = F = 1.

To o
```{r precision_recal}
get_pr <- function(mod) {
   pr_data <- pr.curve(mod[["pred"]][["pred"]], mod[["pred"]][["obs"]], curve = TRUE)  # prec curve using the predition and observations of the model
...
# one of the variables is  equal the classifier
...
resamps <- resamples(list(gbmFit1,))
