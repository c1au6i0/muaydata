...
```{r save_mod1, eval = FALSE, include=FALSE}
...
```{r mod2_smote}
set.seed(1512)
smote_heal <- SMOTE(H4TO117 ~., data = as.data.frame(heal_sub_cl))

table(smote_heal$H4TO117)

mod2_smote <- train(H4TO117  ~ ., 
              smote_heal,
              method = thresh_code,
              ## Minimize the distance to the perfect model
              metric = "Dist",
...
```{r save_mod2, eval = FALSE, include=FALSE}
saveRDS(mod2_smote, "mod2.RDS")

mod2_smote <- readRDS("mod2.RDS")
```
...
library(reshape2)
metrics <- mod2_smote$results[, c(2, 4:6)]
metrics <- melt(metrics, id.vars = "threshold", 
                variable.name = "Resampled",
                value.name = "Data")
...
```{r save_mod3, eval = FALSE, include=FALSE}
saveRDS(mod3_up, "mod3.RDS")

mod3_up <- readRDS("mod3.RDS")
```
...
library(reshape2)
metrics <- mod3_up$results[, c(2, 4:6)]
metrics <- melt(metrics, id.vars = "threshold", 
                variable.name = "Resampled",
                value.name = "Data")
...
```
```{r test_m3}
testIndex <- createDataPartition(heal_sub_cl$H4TO117, p = .2, 
                                  list = FALSE, 
                                  times = 1)

test_heal <-heal_sub_cl[testIndex,]


prova <- predict(mod3_up, newdata = test_heal)

confusionMatrix(prova, test_heal$H4TO117)
```

```{r}
model_list <- list( no_resample = mod1,
                    smote = mod3_up)

resamps <- resamples(model_list)


dotplot(resamps, metric = c("Dist", "ROC"), xlim = c(-0.2, 1.2))
 

plot(mod3_up, metric = "ROC")
...
```
https://rdrr.io/cran/caret/man/prcomp.resamples.html