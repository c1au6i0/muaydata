...
## ROC curves and ```precrec```

When we trained our models we selected ```ROC``` as the metric for choosing the optimal model. ```ROC``` is the Area Under the Curve of the [Receiving Operating Characteristic](https://en.wikipedia.org/wiki/Receiver_operating_characteristic). The ROC curve is generated  by varying the cutoff probabilty for assigning the prediction values ("Yes" or "No") and by plotting the Sensitivity of the model ( TruePositives/ (TruePositive + FalseNegatives) ) as a function of  1 - Specificity ( TrueNegatives/ (TrueNegatives + FalsePositives) ). In other words, the ROC curve plots the rate of True Positives (over the total positive) as a function of the False Allarms ( identified "Yes" even if "No").

To calculate the data for the curves we will use the package ```precrec```. The [package](https://cran.r-project.org/web/packages/precrec/index.html) has been recently listed in CRAN and has same very handy functions to calculate (and plot) ROC and Precision-Recall curves of N-fold cross validation datasets. We will create our own function to extract the data from multiple models using ```precrec::mmdata``` and ```precrec::eval_mod``` and  apply it to the list of models.
...
str(data_rocpr)
```
We obtain a dataframe ```data_rocpr``` with the Y and X coordinates for plotting ROC or PRC.
...
A perfect model tha is able to capture alla the "yes"  and all the "No" will have Sensitivity = Specificity = 1, and this  will generate a curve that pass through the upper left corner and continuing parallel to the x-axes reach the top-right corner. In the other hand, a model that gives random predictions will generate a curve that will lie in vicinity the dotted line. Of course we might have also a model that make predictions worse than random, in that case the curve will be to the right of the dotted line.

## Precision-Recall Curve

A [good argument](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC4349800/) has been made that the ROC curve can be misleading when dealing with very unballanced classes. When the negative class is very small, the probability of making FalsePositive predictions increases. Becouse FalsePositive might still be a small percentage of  the indentified TrueNegative, the specificity of the model would be only minimally affected by such class unballance. Precision-Recall Curves, introduce a different measure, that take into account FalsePositives in the context of the positive predictions: the precision.

The *precision* is the ratio of identified TruePositives / (TruePositives+FalsePositives), and thus indicates how likely is that a "Yes" predition is correct. Of course a model may be still 100 % correct when it predicts "Yes" but be able to identify _*very few*_  "Yes". In that particular case the Recall (aka Sensitivity: TruePositives/ (TruePositive + FalseNegatives) ) of the model will be very low. The best possible model is able to correctly identify all the "Yes", and so has Precision = Recall = F (the harmonic mean of the 2 measures) = 1.

[Precision-Recall](https://ils.unc.edu/courses/2013_spring/inls509_001/lectures/10-EvaluationMetrics.pdf) curves are generated by retriving different percentages of the samples and plotting (for each of the retrived samples) those two metrics, the Precisions against the Recall. Remember that perfection would generate a precision-recall curve that pass to the up-right corner of the grapha and thus have Precision = Recall = F = 1.

```{r precision_recal_alt, include=TRUE ,eval=TRUE}
data_rocpr <- map_dfr(model_list, get_rocpr)

data_rocpr  %>% 
  filter(type == "PRC") %>% 
  mutate(modname = factor(modname, levels = names_mod)) %>%
  ggplot(aes(y = y, x = x, col = modname)) +
  geom_line() +
...
## Putting all together Sensitivity, Specificity

```{r ROC_sens_spec, eval=TRUE, include=TRUE, warning=FALSE}
...
get_metrics <- function(mod, metric = "No") {
  
  # this is just to decide if name the model + the metric used for 
  # selecting hyperparameters or just the model
  if (metric != "No") {
      modname <-  paste0(mod[["method"]],"-",mod[["metric"]])
  } else {
      modname <-  mod[["method"]]
  }
...
```{r plot_pr, warning=FALSE}
metrics %>% 
    mutate(model = factor(model, levels = c("glmnet", "rpart", "ctree", "rf"))) %>% 
    filter(metric %in% c("AUC", "F", "Precision", "Recall")) %>% 
    ggplot(aes(y= mean, x = model, col = model)) +
    geom_point(size = 2.5) +
    geom_errorbar(aes(ymin = mean - sd, ymax =  mean + sd), width = 0.1) +
    scale_y_continuous(limits = c(0, 1)) + 
